...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
      diff = scores[i] - np.max(scores[i]) 
      softmax = np.exp(diff)/np.sum(np.exp(diff))
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    sum_exp_score = np.exp(score).sum(axis=1,keepdims=True)
    softmax_matrix = np.exp(score)/sum_exp_score
