...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.dot(z_i.view(-1), z_j.view(-1)) / (norm_i * norm_j)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # --- Loss for Left View (k) ---
        # Numerator: exp(sim(left_k, right_k) / tau)
        numerator_k = torch.exp(sim(z_k, z_k_N) / tau)
        
        denominator_k = 0
        for j in range(2 * N):
            if k != j: # Sum over all j except itself
                denominator_k += torch.exp(sim(z_k, out[j]) / tau)
        
        loss_k = -torch.log(numerator_k / denominator_k)

        # --- Loss for Right View (k+N) ---
        # Numerator: exp(sim(right_k, left_k) / tau)
        numerator_kN = torch.exp(sim(z_k_N, z_k) / tau)
        
        denominator_kN = 0
        for j in range(2 * N):
            if (k + N) != j: # Sum over all j except itself
                denominator_kN += torch.exp(sim(z_k_N, out[j]) / tau)
                
        loss_kN = -torch.log(numerator_kN / denominator_kN)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # Normalize vectors along dimension 1 (D)
    out_left_norm = torch.nn.functional.normalize(out_left, p=2, dim=1)
    out_right_norm = torch.nn.functional.normalize(out_right, p=2, dim=1)
    
    # Element-wise multiplication and sum along dimension 1 gives dot product of normalized vectors
    # Shape: (N, D) * (N, D) -> (N, D) -> sum -> (N, 1)
...
def compute_sim_matrix(out):
    ...
    # 1. Normalize the input vectors to have unit length
    out_norm = torch.nn.functional.normalize(out, p=2, dim=1)
    
    # 2. Perform matrix multiplication: (2N x D) @ (D x 2N) = (2N x 2N)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Calculate sim for (left_i, right_i). This covers indices [0...N-1]
    pos_sim_left = sim_positive_pairs(out_left, out_right)
    # Concatenate with itself because (right_i, left_i) has the same similarity (cosine is symmetric)
    # This covers indices [N...2N-1]
    pos_sim = torch.cat([pos_sim_left, pos_sim_left], dim=0) 
