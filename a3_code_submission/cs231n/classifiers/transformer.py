...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # Project image features to wordvec_dim and repeat across time
        img_embed = self.visual_projection(features)                 # (N, W)
        img_embed = img_embed.unsqueeze(1).repeat(1, T, 1)          # (N, T, W)

        # The decoder expects memory (from encoder) to be the image features
        memory = img_embed                                                  # (N, T, W)  -> used as K/V in cross-attention

        # Create causal mask so captions can't attend to future tokens
        # mask shape: (T, T), lower triangle = True, upper = False
        mask = torch.tril(torch.ones(T, T, device=captions.device)) == 1
        # We need to broadcast mask properly: (T, T) -> (N*T, T) in decoder internals
        # But TransformerDecoder from the assignment handles (T, T) mask correctly

        # Run through the Transformer decoder (self-attention + cross-attention to image)
        decoder_out = self.transformer(
            tgt=caption_embed,      # queries come from caption tokens
            memory=memory,          # keys/values come from image features
            tgt_mask=mask           # causal mask for autoregressive generation
        )                                                                   # (N, T, W)

        # Final linear layer to vocabulary scores
