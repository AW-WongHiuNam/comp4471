...
def rnn_step_backward(dnext_h, cache):
    ...
    # Compute local gradient of tanh
...
def rnn_backward(dh, cache):
    ...
    # Unpack dimensions
    N, T, H = dh.shape
    # Get D from the first cache's x: cache[0][0].shape = (N, D)
    D = cache[0][0].shape[1]
    
    # Initialize gradients
    dx = np.zeros((N, T, D))
    dWx = np.zeros_like(cache[0][2])  # (D, H)
    dWh = np.zeros_like(cache[0][3])  # (H, H)
    db = np.zeros((H,))
    dprev_h = np.zeros((N, H))  # Carry gradient to previous h, init to 0 (no future)
    
    # Backprop through time: loop backwards
    for t in range(T - 1, -1, -1):
        # The dnext_h includes the upstream dh[:, t, :] plus the gradient from next timestep
        dnext_h = dh[:, t, :] + dprev_h
        # Compute step gradients
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    do = dnext_h * tanh_c
    dtanh_c = dnext_h * o
    dnext_c = dtanh_c * (1 - tanh_c**2) + dnext_c
